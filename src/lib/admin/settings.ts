...
export async function getTableFields(modelName: string): Promise<AdminField[]> {
  ...
  return model.fields.filter((f) => f.read).sort((a, b) => a.order - b.order);
}
...
export async function getCreateFields(
  ...
  return model.fields.filter((f) => f.create).sort((a, b) => a.order - b.order);
}
...
export async function getUpdateFields(
  ...
  return model.fields.filter((f) => f.update).sort((a, b) => a.order - b.order);
}
