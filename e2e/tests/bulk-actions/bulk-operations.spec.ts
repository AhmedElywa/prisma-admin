...
test.describe('Bulk Actions and Confirmations', () => {
  ...
  test('should show bulk actions button when rows selected', async ({
    page,
  }) => {
  ...
  test('should handle keyboard shortcuts for selection', async ({ page }) => {
    ...
    const checkedCount = await listPage.tableRows
  ...
  test('should disable bulk actions for protected items', async ({ page }) => {
    ...
    const protectedRows = listPage.tableRows.filter({
      has: page.locator('[data-protected="true"]'),
    });
  ...
  test('should show confirmation for dangerous bulk actions', async ({
    page,
  }) => {
    await listPage.selectAllRows();
    await listPage.bulkActionsButton.click();
