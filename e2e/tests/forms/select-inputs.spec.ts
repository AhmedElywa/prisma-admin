...
test.describe('Select/Dropdown Fields', () => {
  ...
  test('should display select fields', async ({ page }) => {
    ...
    const selectField = formPage.form
  ...
  test('should handle multi-select if available', async ({ page }) => {
    const multiSelect = formPage.form
      .locator(
        '[role="combobox"][aria-multiselectable="true"], select[multiple]'
      )
      .first();
  ...
  test('should validate required select fields', async ({ page }) => {
    ...
    const requiredSelect = formPage.form
      .locator('select[required], [role="combobox"][aria-required="true"]')
      .first();
  ...
  test('should group options if categories exist', async ({ page }) => {
    ...
    if (await selectField.isVisible()) {
      ...
      if ((await optionGroups.count()) > 0) {
        ...
        const groupLabel = await optionGroups
          .first()
  ...
  test('should clear selection with clear button', async ({ page }) => {
    ...
    if (await selectField.isVisible()) {
      ...
      const clearButton = selectField.locator(
  ...
  test('should load options asynchronously', async ({ page }) => {
    ...
    const asyncSelect = formPage.form
      .locator('[role="combobox"][aria-busy]')
      .first();
  ...
  test('should handle option with icons or badges', async ({ page }) => {
    ...
    if (await selectField.isVisible()) {
      ...
      const optionWithBadge = page
        .locator('[role="option"] [class*="badge"]')
        .first();

      if (
        (await optionWithIcon.isVisible()) ||
        (await optionWithBadge.isVisible())
      ) {
