...
test.describe('Date/Time Input Fields', () => {
  ...
  test('should display date input fields', async ({ page }) => {
    const dateFields = formPage.form.locator(
  ...
  test('should open date picker on click', async ({ page }) => {
    ...
    if (await dateField.isVisible()) {
      ...
      const calendar = page.locator(
        '[role="dialog"][aria-label*="calendar"], .calendar-popup, [class*="datepicker"]'
      );
  ...
  test('should handle datetime-local input', async ({ page }) => {
    const datetimeField = formPage.form
  ...
  test('should respect min/max date constraints', async ({ page }) => {
    ...
    if (await dateField.isVisible()) {
      ...
      const _max = await dateField.getAttribute('max');
  ...
  test('should show today button in picker', async ({ page }) => {
    ...
    if (await dateField.isVisible()) {
      ...
      const todayButton = page.locator(
        'button:has-text("Today"), button[aria-label*="today"]'
      );
  ...
  test('should navigate months in date picker', async ({ page }) => {
    ...
    if (await dateField.isVisible()) {
      ...
      const prevMonth = page
        .locator('button[aria-label*="previous month"], button:has-text("<")')
        .first();
      const nextMonth = page
        .locator('button[aria-label*="next month"], button:has-text(">")')
        .first();
  ...
  test('should clear date field', async ({ page }) => {
    ...
    if (await dateField.isVisible()) {
      ...
      const clearButton = dateField.locator(
  ...
  test('should validate future/past date requirements', async ({ page }) => {
    const publishDateField = formPage.form
      .locator('input[name*="publish"], input[placeholder*="publish"]')
      .first();
    ...
    if (await publishDateField.isVisible()) {
      ...
      const _error = await formPage.getErrorMessage();
  ...
  test('should integrate with calendar libraries', async ({ page }) => {
    ...
    if (await dateField.isVisible()) {
      ...
      const customCalendar = page.locator(
        '.react-datepicker, .react-calendar, [class*="calendar"]'
      );
      ...
      if (await customCalendar.isVisible({ timeout: 1000 })) {
        ...
        const yearSelector = customCalendar.locator(
          'select[aria-label*="year"]'
        );
        const _monthSelector = customCalendar.locator(
  ...
  test('should handle date ranges if applicable', async ({ page }) => {
    ...
    const startDateField = formPage.form
      .locator('input[name*="start"], input[placeholder*="from"]')
      .first();
    const endDateField = formPage.form
      .locator('input[name*="end"], input[placeholder*="to"]')
      .first();

    if (
      (await startDateField.isVisible()) &&
      (await endDateField.isVisible())
    ) {
      // Set start date
  ...
  test('should show proper ARIA labels', async ({ page }) => {
    ...
    if (await dateField.isVisible()) {
      ...
      const labelText = await formPage.form
        .locator(`label[for="${await dateField.getAttribute('id')}"]`)
        .textContent();
