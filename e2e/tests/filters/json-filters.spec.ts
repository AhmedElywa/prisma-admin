...
test.describe('JSON Filter Tests', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    const testPosts = [
      {
        ...
        metadata: { tags: ['tech', 'news'], views: 100 },
      },
      {
      ...
      },
      {
        ...
        metadata: { settings: { featured: true, priority: 'high' } },
      },
    ...
    for (const post of testPosts) {
      ...
      // Handle JSON field
      const jsonEditor = formPage.form
        .locator(
          '[data-testid*="json"], .json-editor, textarea[name*="metadata"]'
        )
        .first();
  ...
  test('should filter JSON by key-value pair', async ({ page }) => {
    await filterPanel.open();
    await filterPanel.addJsonFilter(
      'Metadata',
      'path equals',
      'author',
      'John Doe'
  ...
  test('should use JSON path syntax', async ({ page }) => {
    ...
    const pathInput = filterPanel.panel
      .locator('input[placeholder*="path"], input[name*="path"]')
      .first();
  ...
  test('should validate JSON syntax in filter', async ({ page }) => {
    ...
    // Try invalid JSON
    const jsonInput = filterPanel.panel
  ...
  test('should handle null JSON values', async ({ page }) => {
    ...
    const jsonEditor = formPage.form
      .locator('[data-testid*="json"], .json-editor')
      .first();
    if (await jsonEditor.isVisible()) {
  ...
  test('should provide JSON filter templates', async ({ page }) => {
    ...
    const templateSelect = filterPanel.panel.locator(
      'select[name*="template"], button:has-text("Templates")'
    );
    if (await templateSelect.isVisible()) {
      ...
      expect(
        templateTexts.some((t) => t.match(/key|value|array|nested/i))
      ).toBe(true);
    }
  });
  ...
  test('should handle JSON filter with special characters', async ({
    page,
  }) => {
    ...
    if (await jsonEditor.isVisible()) {
      await jsonEditor.fill(
        JSON.stringify({ 'key with spaces': 'value', 'special@char': true })
      );
    }
