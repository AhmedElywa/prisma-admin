...
test.describe('Relation Filter Tests', () => {
  ...
  test('should filter by specific relation', async ({ page }) => {
    ...
    const authorFilter = filterPanel.panel
    ...
    if (await authorFilter.isVisible()) {
      ...
      const authorOptions = page
        .locator('[role="option"], .dropdown-item')
        .filter({ hasText: /alice|bob/i });
  ...
  test('should filter by relation count', async ({ page }) => {
    ...
    const postCountFilter = filterPanel.panel
  ...
  test('should filter by nested relation', async ({ page }) => {
    ...
    const nestedFilter = filterPanel.panel
      .locator('input')
      .filter({ hasText: /author.*name/i })
      .first();
  ...
  test('should use relation search/select UI', async ({ page }) => {
    ...
    if (await fieldSelect.isVisible()) {
      ...
      const relationPicker = filterPanel.panel
        .locator(
          '[data-testid*="relation"], .relation-picker, [role="combobox"]'
        )
        .first();
  ...
  test('should filter many-to-many relations', async ({ page }) => {
    ...
    const categoryFilter = filterPanel.panel
      .locator('select, input')
  ...
  test('should filter by relation ID', async ({ page }) => {
    ...
    const authorIdFilter = filterPanel.panel
      .locator('input')
      .filter({ hasText: /author.*id/i })
      .first();
  ...
  test('should combine relation filters with other filters', async ({
    page,
  }) => {
    await filterPanel.open();
  ...
  test('should show relation details in filter', async ({ page }) => {
    ...
    const relationValue = filterPanel.panel
      .locator('.relation-value, [data-testid*="relation-value"]')
      .first();
  ...
  test('should handle self-referencing relations', async ({ page }) => {
    ...
    const parentFilter = filterPanel.panel
      .locator('select, input')
  ...
  test('should filter by relation with complex conditions', async ({
    page,
  }) => {
    await filterPanel.open();

    // Filter posts where author name starts with 'A'
    const complexFilter = filterPanel.panel
      .locator('button')
      .filter({ hasText: /advanced|complex/i })
      .first();
  ...
  test('should show loading state for relation options', async ({ page }) => {
    ...
    const loading = filterPanel.panel.locator(
  ...
  test('should paginate relation options if many', async ({ page }) => {
    ...
    const relationPicker = filterPanel.panel
      .locator('[data-testid*="relation"], .relation-picker')
      .first();
    if (await relationPicker.isVisible()) {
      await relationPicker.click();
  ...
  test('should handle circular relations gracefully', async ({ page }) => {
    ...
    const circularFilter = filterPanel.panel
      .locator('select')
      .filter({ hasText: /author.*posts.*author/i })
      .first();
