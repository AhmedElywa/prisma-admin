...
test.describe('Enum Filter Tests', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    for (const user of testUsers) {
      ...
      const roleSelect = formPage.form
        .locator('select[name*="role"], [role="combobox"][name*="role"]')
        .first();
      if (await roleSelect.isVisible()) {
        if ((await roleSelect.getAttribute('role')) === 'combobox') {
          await roleSelect.click();
          await page
            .locator(`[role="option"]:has-text("${user.role}")`)
            .click();
        } else {
  ...
  test('should filter by multiple enum values (in operator)', async ({
    page,
  }) => {
    await filterPanel.open();
  ...
  test('should handle empty enum values', async ({ page }) => {
    ...
    if (await roleSelect.isVisible()) {
      ...
      const emptyOption = roleSelect.locator(
        'option[value=""], option:has-text("Select")'
      );
  ...
  test('should handle enum with many values', async ({ page }) => {
    ...
    const valueSelect = filterPanel.panel
  ...
  test('should handle enum filter in bulk operations', async ({ page }) => {
    ...
    // Select all filtered
    const selectAll = listPage.table
  ...
  test('should show enum filter in active filters', async ({ page }) => {
    ...
    const activeFilter = page
      .locator('[data-testid="active-filter"], .active-filter')
      .filter({ hasText: /role.*admin/i });
  ...
  test('should handle dynamic enum loading', async ({ page }) => {
    ...
    const loadingIndicator = filterPanel.panel.locator(
