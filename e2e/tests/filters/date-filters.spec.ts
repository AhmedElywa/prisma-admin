...
test.describe('Date Filter Tests', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    for (const post of testPosts) {
      ...
      const dateField = formPage.form
        .locator(
          'input[type="datetime-local"][name*="publish"], input[type="date"][name*="publish"]'
        )
        .first();
  ...
  test('should filter with "between" date range', async ({ page }) => {
    await filterPanel.open();
    await filterPanel.addRangeFilter(
      'Published At',
      'between',
      '2024-01-01',
      '2024-12-31'
  ...
  test('should filter with relative dates', async ({ page }) => {
    ...
    const relativeOptions = filterPanel.panel
      .locator('button, select option')
      .filter({ hasText: /today|yesterday|week|month/i });
  ...
  test('should handle datetime with time component', async ({ page }) => {
    ...
    const datetimeInput = filterPanel.panel
  ...
  test('should filter by year only', async ({ page }) => {
    ...
    const yearSelect = filterPanel.panel
  ...
  test('should filter by month', async ({ page }) => {
    ...
    const monthSelect = filterPanel.panel
  ...
  test('should show calendar widget', async ({ page }) => {
    ...
    if (await dateInput.isVisible()) {
      ...
      const calendar = page.locator(
        '[role="dialog"][aria-label*="calendar"], .calendar-popup'
      );
