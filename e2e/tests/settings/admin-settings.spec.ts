...
test.describe('Admin Settings', () => {
  ...
  test('should save settings changes', async ({ page }) => {
    ...
    if (await saveButton.isVisible()) {
      ...
      await expect(
        page.locator('[role="status"]').filter({ hasText: /saved|success/i })
      ).toBeVisible();
    }
  });
  ...
  test('should show general settings section', async ({ page }) => {
    ...
    const itemsPerPageSetting = page
      .locator('label')
      .filter({ hasText: /items per page/i });
